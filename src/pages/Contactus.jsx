...
function Contactus() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.name || !formData.email || !formData.subject || !formData.message || !formData.company ||
       !formData.website || !formData.phone || !formData.industry) {
      alert('Please fill in all required fields.');
      return;
    }

    if (!validatePhone(formData.phone)) {
      alert('Please enter a valid phone number (only digits, 10 to 15 characters).');
      return;
    }
    ...
    try {
      const response = await fetch(`${baseUrl}/Website_Contact_PHP/contact.php`, {
        method: 'POST',
        headers: {
  ...
  <FloatingLabel
                    ...
                    <div class="col-md-6 col-sm-6 col-xs-12">
                      <div className="single-input-field d-flex">
                          ...
                          title={
                            selectedCountry ? (
                              <div style={{ display: 'flex', alignItems: 'center' }}>
                                <img src={selectedCountry.flag} alt={selectedCountry.name} width="20" height="15" style={{ marginRight: '10px' }} />
                        ...
                        >
                          ...
                          {loading ? (
                            <Dropdown.Item>Loading country codes...</Dropdown.Item>
                          ) : (
                            filteredCountries.length > 0 ? (
                              filteredCountries.map((country, index) => (
                                <Dropdown.Item
                                  key={index}
                                  onClick={() => handleCountrySelect(country.code, country.name, country.flag)}
                                >
                                  <div style={{ display: 'flex', alignItems: 'center' }}>
                                    <img
                                      src={country.flag}
      ...
      >
        <Form.Control type="tel"  name="phone"
                          value={formData.phone} style={{width:'320px',height:"20px"}}
                          onChange={handleInputChange} placeholder="name@example.com" />
      </FloatingLabel>
